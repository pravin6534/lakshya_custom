...
def execute(filters: Filters | None = None) -> tuple:
    ...
    if not (filters.from_date and filters.to_date):
        frappe.throw(_("Please select a valid date range (From Date and To Date)."))
...
def get_columns_for_days(filters: Filters) -> list[dict]:
    """
    Returns day-wise columns for the given date range (from_date to to_date).
    """
    from_date = getdate(filters.from_date)
    to_date = getdate(filters.to_date)
    total_days = get_total_days_in_date_range(filters)

    days = []
    for day_offset in range(total_days):
        current_date = from_date + timedelta(days=day_offset)
        day = cstr(current_date.day)  # Extract day of the month
        weekday = day_abbr[current_date.weekday()]  # Get weekday abbreviation
        label = f"{day} {weekday}"  # e.g., "1 Mon"
        days.append({"label": label, "fieldtype": "Data", "fieldname": day, "width": 65})

    return days

def get_dates_in_range(from_date: str, to_date: str) -> list[str]:
    """
    Returns a list of all dates between from_date and to_date (inclusive).
    """
...
def get_total_days_in_date_range(filters: Filters) -> int:
    """
    Returns the total number of days in the given date range (from_date to to_date).
    """
    if not filters.from_date or not filters.to_date:
        frappe.throw(_("Please provide both From Date and To Date in the filters."))
    ...
    return (to_date - from_date).days + 1  # Inclusive of both dates
...
def get_holiday_map(filters: Filters) -> dict[str, list[dict]]:
    """
    Returns a dict of holidays in the given date range (from_date to to_date).
    """
    # Fetch holiday lists
    ...
    for d in holiday_lists:
        ...
        holidays = (
            ...
            .where(
                (Holiday.parent == d)
                & (Holiday.holiday_date >= filters.from_date)
                & (Holiday.holiday_date <= filters.to_date)
            )
        ).run(as_dict=True)
...
def get_attendance_status_for_detailed_view(
    ...
    [
        {'shift': 'Morning Shift', 1: 'A', 2: 'P', ..., 'total_days_in_month': 30, 'total_present_days': 20, 'total_absent_days': 10},
        {'shift': 'Evening Shift', 1: 'P', 2: 'A', ..., 'total_days_in_month': 30, 'total_present_days': 15, 'total_absent_days': 15},
    ]
    """
    from_date = getdate(filters.from_date)
    to_date = getdate(filters.to_date)
    total_days = get_total_days_in_date_range(filters)  # Total days in the selected date range
    attendance_values = []
    ...
    for shift, status_dict in employee_attendance.items():
        ...
        for day_offset in range(total_days):
            current_date = from_date + timedelta(days=day_offset)
            day = int(current_date.day)  # Extract day of the month
            status = status_dict.get(day)
            if status is None and holidays:  # Check for holidays if status is None
                status = get_holiday_status(day, holidays)
        ...
        # Calculate total absent days as total days - total present days
        total_absent_days = total_days - total_present_days

        # Add calculated fields to the row
        row["total_days_in_month"] = total_days  # Total days in the date range
        row["total_present_days"] = total_present_days  # Total present days
        row["total_absent_days"] = total_absent_days  # Total absent days
